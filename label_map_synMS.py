...
def label_map_synLes(label_map, points,dict_point_clés, facteur_confluence, template_p_T1, nb_lesions):
        ...
        while(bool == False and attempt < max_attempt):
            ...
            while (dossier_assoc_num == [] and iteration < max_iter): #pas de lésions dans ce dossier 
                ...
                while good_cand == False :
                    ...
                    if(ventricule_mask[candidate[0], candidate[1], candidate[2]] == 0 and cortex_mask[candidate[0], candidate[1], candidate[2]] == 0 and lesion_mask[candidate[0], candidate[1], candidate[2]]== 0 and label_MNI[candidate[0], candidate[1], candidate[2]]!= 0 and mask_synth_lesions[candidate[0], candidate[1], candidate[2]] == 0):
                        good_cand = True
    ...
    output_Newmask = os.path.join(dossier_new_mask, f'mask_lesion_instantiel_{name}.nii.gz')
    cmd = f"antsApplyTransforms -d 3 -i {lesion_mask_p} -r {label_map} -n genericLabel -t [{backward_transforms[0]},1] -t {backward_transforms[1]} -o {output_Newmask}"
    subprocess.Popen(cmd, shell = True).wait()
...
if __name__ == "__main__":
   ...
   # nb_image_traitées = len(label_map)
    ...
    for map in range(nb_image_traitées):
        ...
        output_label_mask = label_map_synLes(label_map[map], points, dict_point_clés, facteur_confluence, template_p_T1, 150)
