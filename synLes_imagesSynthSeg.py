...
#     subprocess.Popen(cmd, shell = True).wait()
    aligned_image = ants.image_read(label_map)
    image_p = label_map
    ...
    for i in range(nb_lesions):
        ...
        max_attempt = 200
        attempt = 0
        cortex_mask = np.isin(label_MNI, [3,8,42,47]).astype(np.uint8)
        brain_mask = np.isin(label_MNI, [507,511,515,516,514,502,509,24]).astype(np.uint8) 
        ventricule_mask = np.isin(label_MNI, [4,5,14,15,43,44,10,49,28,60]).astype(np.uint8)
        ...
        while(bool == False and attempt < max_attempt):
            ...
            while (dossier_assoc_num == [] and iteration < max_iter): #pas de lésions dans ce dossier 
                ...
                while good_cand == False :
                    ...
                    if(ventricule_mask[candidate[0], candidate[1], candidate[2]] == 0 and cortex_mask[candidate[0], candidate[1], candidate[2]] == 0 and label_MNI[candidate[0], candidate[1], candidate[2]]!= 0 and mask_synth_lesions[candidate[0], candidate[1], candidate[2]] == 0 and brain_mask[candidate[0], candidate[1], candidate[2]] == 0 and lesion_mask[candidate[0], candidate[1], candidate[2]]==0) :
                        good_cand = True
        ...
        if(attempt == max_attempt):
            ...
            while(np.isin(label_MNI, label).astype(np.uint8).any()):
                label +=1
            a = np.sum(new_mask_lesion[left_brain_mask>0])
            b = np.sum(new_mask_lesion[right_brain_mask>0])
            if(a>b):
                label2 = 25
                new_mask_lesion[right_brain_mask>0] = 0
            else :
                label2 = 57
                new_mask_lesion[left_brain_mask>0] = 0
                
            
            label_MNI[new_mask_lesion>0] = np.uint8(label2)
            mask_synth_lesions[new_mask_lesion>0] = np.uint8(label2)
            label_MNI_lesion_sem[new_mask_lesion>0] = np.uint8(label2)
            mask_synth_lesions_binary[new_mask_lesion>0] = np.uint8(label2)
            
                
    lesion_mask_nib = nib.Nifti1Image(mask_synth_lesions,template.affine)
    label_MNI_nib =  nib.Nifti1Image(label_MNI,template.affine)
    ...
    nib.save(lesion_mask_nib,f'/home/jdrochmans/data/juliette/reg/mask_synLes_synthseg_instance_{name}.nii.gz')
    nib.save(label_MNI_nib,f'/home/jdrochmans/data/juliette/label/label_MNI_{name}.nii.gz')
    ...
    label_MNI_p = f'/home/jdrochmans/data/juliette/label/label_MNI_{name}.nii.gz'
...
if __name__ == "__main__":
   ...
   # nb_image_traitées = len(label_map)
    nb_image_traitées = 20
    for map in range(nb_image_traitées):
        
        path_inst, name = label_map_synLes(label_map[map], points, dict_point_clés, facteur_confluence, template_p_T1, 50)
        print(path_inst)
